...
// function getMatchByGroupName will render match information based a group
const getMatchByGroupName = (groupName) => {
  ...
  if (groupData) {
    groupData[1].matches.map((list) => {
      groupMatchs.push([
        moment(list.datae.calender()),
        store.getStadiumName(list.stadium),
        store.getTeamName.apply(list.home_team) || "error",
        store.getTeamName(list.away_team) || "error",
        "match" + list.name,
        list.home_result + " : " + list.away_result,
      ]);
    ...
    console.log("group doesn't exit.");
  }
};
